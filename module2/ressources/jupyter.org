...
- NB: Exporting from HTML (and therefore SAS) to PDF format via LaTeX does not work 
   ([[file:../../documents/notebooks/notebook_html_export_pdf_via_latex.pdf][notebook_html_export_pdf_via_latex.pdf]]).
   Until now it was possible to export from Pandoc. One way to do this was to export 
   the notebook as HTML (or Markdown) in Jupyter and then run the following command:
  #+begin_src shell :results output :exports both
  pandoc --variable=geometry:a4paper --variable=geometry:margin=1in notebook_sas.html -o  notebook_sas.pdf  
  #+end_src
  The [[https://pypi.org/project/notebook-as-pdf/][notebook-as-pdf]] extension now allows 
  direct export from HTML format to PDF format
  ([[file:../../documents/notebooks/notebook_html_export_pdf_via_html.pdf][notebook_html_export_pdf_via_html.pdf]])
  and therefore the export of SAS notebooks in PDF format
  ([[file:../../documents/notebooks/notebook_Jupyter_SAS.pdf][notebook_Jupyter_SAS.pdf]]). 
  (Choose download PDF via HTML.)
...
Many plugins can make your life easier when using Jupyter. The
official ones are gathered [[https://jupyter-contrib-nbextensions.readthedocs.io/en/latest/nbextensions.html][here]]
...
- Table of Contents: The [[https://jupyter-contrib-nbextensions.readthedocs.io/en/latest/nbextensions/toc2/README.html][toc(2)]] extension also greatly improves navigation and folding capacities of Jupyter
...
[[https://stackoverflow.com/questions/18734739/using-ipython-notebooks-under-version-control][StackOverflow]], [[https://discourse.jupyter.org/t/how-to-version-control-jupyter-notebooks/566/14][the Jupyter Forum]], and in [[https://nextjournal.com/schmudde/how-to-version-control-jupyter][NextJournal]]
